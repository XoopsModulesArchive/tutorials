...
OpenTable();
$result = $xoopsDB->query('select tlink from ' . $xoopsDB->prefix('tutorials') . " where tid=$tid ");
[$tlink] = $xoopsDB->fetchRow($result);
$tlinkstring = $tlink;
if (mb_strlen($tlinkstring) > 50) {
    $tlinkstring = mb_substr($tlinkstring, 0, 50) . '...';
}
